...
struct Tree
{
	...
	uint8_t IntervaToMeasure = 1;
	uint16_t IntervalToWater = 10;
	uint16_t MinHumidity = 19;
} Trees[3];
...
char TreeInfos[3][20] = { "7:30;1;5;20", "7:30;1;5;20", "7:30;1;5;20" };  /////
...
boolean IsShouldBeWatered = false;
boolean IsEndMeasuring[6] = { false, false, false, false, false, false };
boolean IsEndWatering[6] = { false, false, false, false, false, false };
...
void setup()
{
	...
	serialCMD.AddCommand("t1", TreeInfos[0]); //t1-10:30;2;5;20
	serialCMD.AddCommand("t2", TreeInfos[1]); //t2-10:30;2;5;20
	serialCMD.AddCommand("t3", TreeInfos[2]); //t3-10:30;2;5;20
...
void UpdateData()
{
	for (int i = 0; i < 3; i++)
	{
		...
		Serial.println(Trees[i].IntervaToMeasure);
		Serial.println(Trees[i].IntervalToWater);
		Serial.println(Trees[i].MinHumidity);
	}
...
void ParseTreeData(uint8_t index)  /////
{
	String hour;
	String min;
	String intervalMeasure;
	String intervalWater;
	String minHumidity;
	...
	semicon[2] = s.indexOf(';', semicon[1]);

	String time = s.substring(0, semicon[0]);
	hour = time.substring(0, time.indexOf(':'));
	min = time.substring(time.indexOf(':') + 1);

	intervalMeasure = s.substring(semicon[0] + 1, semicon[1]);
	intervalWater = s.substring(semicon[1] + 1, semicon[2]);
	minHumidity = s.substring(semicon[2] + 1);

	Trees[index].TimeToWater.Hour = hour.toInt();
	Trees[index].TimeToWater.Minute = min.toInt();
	Trees[index].IntervaToMeasure = intervalMeasure.toInt();
	Trees[index].IntervalToWater = intervalWater.toInt();
	Trees[index].MinHumidity = minHumidity.toInt();
}
...
void CheckForWater()
{
	...
	for (int i = 0; i < GardenTower.Length; i++)
	{
		...
		if (st == delta)
		{
			...
			IsEndWatering[(int)PotIndex] = true;

			delay(500);
		}
		else
		{
			IsEndWatering[(int)PotIndex] = false;
		}

		if (delta != 0 && delta % tree.IntervaToMeasure == 0)
		{
			HandServo.write(HAND_SERVO_CLOSE);
			ArmServo.write(ARM_SERVO_CLOSE);

			if (IsEndMeasuring[(int)PotIndex])
				continue;
			...
			IsEndMeasuring[(int)PotIndex] = true;

			if (t_hum <= tree.MinHumidity)
			{
				Water();
			}
		...
		{
			IsEndMeasuring[(int)PotIndex] = false;
		}
	}
}
